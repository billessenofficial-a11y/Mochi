...
private final class SessionAudioRecorder: @unchecked Sendable {
    ...
    func begin(format: AVAudioFormat) throws -> URL {
        ...
        let url = directory.appendingPathComponent("clearcue-\(UUID().uuidString).caf")
...
final class SpeechRecognitionService: ObservableObject {
    private static let logger = Logger(subsystem: "com.jamesangrellera.clearcue", category: "Speech")
    ...
    func start() async {
        ...
        do {
            ...
            if captionsEnabled, captionEngine == .openAIRealtime {
                ...
                do {
                    let clientSecret = try await ClearCueAPI.shared.createRealtimeClientSecret()
