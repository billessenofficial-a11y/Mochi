...
final class ConversationStore: ObservableObject {
    ...
    func refreshCatchUp(force: Bool = false) {
        ...
        Task { [weak self] in
            guard let self else { return }
            do {
                let generated = try await ClearCueAPI.shared.generateCatchUp(
                    segments: snapshot,
    ...
    private func refineTranscript(
        ...
        do {
            let generated = try await ClearCueAPI.shared.refineRecording(at: recordingURL)
    ...
    private func buildRecap() async {
        ...
        if mode.capturesAudio {
            ...
            do {
                let generated = try await ClearCueAPI.shared.generateRecap(
                    segments: segments,
                    repairs: repairs,
                    userName: userName,
