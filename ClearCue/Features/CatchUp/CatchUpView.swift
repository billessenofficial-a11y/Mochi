...
struct CatchUpView: View {
    ...
    private var overviewCard: some View {
        ...
        .padding(20)
        .background(ClearCueTheme.ink, in: RoundedRectangle(cornerRadius: 24, style: .continuous))
        .shadow(color: ClearCueTheme.ink.opacity(0.14), radius: 18, y: 9)
    }
    ...
    private func foreground(for kind: CatchUpKind) -> Color {
        kind == .needsYou ? ClearCueTheme.amberStrong : ClearCueTheme.ink
    }

    private func background(for kind: CatchUpKind) -> Color {
        kind == .needsYou ? ClearCueTheme.amber : ClearCueTheme.mint
    }
}
