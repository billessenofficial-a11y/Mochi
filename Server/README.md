...
The server reads `OPENAI_API_KEY` from the workspace `.env.local` and listens on port `8787` by default. The iOS Simulator uses `http://127.0.0.1:8787`. For a physical iPhone, set `ClearCueAPIBaseURL` to an HTTPS deployment or a reachable development-machine address.
