...
struct OnboardingView: View {
    ...
    var body: some View {
        ...
        .background {
            LinearGradient(
                colors: [MochiTheme.softMint, MochiTheme.canvas, MochiTheme.canvas],
    ...
    private func readinessRow(_ title: String, detail: String, ready: Bool) -> some View {
        HStack(spacing: 12) {
            Image(systemName: ready ? "checkmark.circle.fill" : "circle.dotted")
                .font(.title3)
                .foregroundStyle(ready ? MochiTheme.ink : MochiTheme.secondaryText)
                .symbolEffect(.bounce, value: ready)
            VStack(alignment: .leading, spacing: 2) {
                Text(title).font(.headline)
                Text(detail).font(.caption).foregroundStyle(MochiTheme.secondaryText)
            }
        }
    }
