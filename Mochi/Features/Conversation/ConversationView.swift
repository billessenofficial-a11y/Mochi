...
struct ConversationView: View {
            ...
            HStack(spacing: 12) {
                VStack(alignment: .leading, spacing: 2) {
                    Text(sessionModeLabel)
                        ...
                        .tracking(1.1)
                        .foregroundStyle(store.mode == .guidedDemo ? MochiTheme.amberStrong : MochiTheme.ink)
                ...
                if store.sessionCaptionsEnabled {
                    Button {
                        ...
                        Label("Speakers", systemImage: "person.2.fill")
                            ...
                            .padding(.horizontal, 13)
                            .frame(height: 42)
                            .background(MochiTheme.surface, in: Capsule())
                            .overlay(Capsule().stroke(MochiTheme.divider.opacity(0.7)))
                    }
    ...
    private var statusStrip: some View {
        HStack(spacing: 8) {
            if store.speechService.isPreparing {
                ...
                Circle()
                    .fill(store.speechService.isListening ? Color.red : MochiTheme.secondaryText)
        ...
        .padding(.vertical, 12)
        .background(MochiTheme.surface, in: RoundedRectangle(cornerRadius: 17, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 17).stroke(MochiTheme.divider.opacity(0.65)))
        .padding(.horizontal, 20)
        ...
        VStack(spacing: 15) {
            ZStack(alignment: .bottomTrailing) {
                Image("MochiMascot")
                    ...
                    .padding(7)
                    .background(MochiTheme.mint.opacity(0.68), in: Circle())

                Image(systemName: store.speechService.errorMessage == nil ? "waveform" : "exclamationmark.triangle.fill")
                    .font(.body.weight(.bold))
                    .foregroundStyle(store.speechService.errorMessage == nil ? MochiTheme.ink : MochiTheme.danger)
                    .frame(width: 40, height: 40)
                    .background(MochiTheme.surface, in: Circle())
                    .overlay(Circle().stroke(MochiTheme.divider.opacity(0.7)))
    ...
    private var controls: some View {
            ...
            if store.sessionCaptionsEnabled {
                Button {
                    ...
                    Label("Catch me up", systemImage: "arrow.uturn.backward.circle.fill")
                        ...
                        .background(MochiTheme.ink, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
                }
            }
        ...
        .padding(.top, 12)
        .padding(.bottom, 14)
        .background(MochiTheme.surface.opacity(0.88))
    }
        ...
        HStack(alignment: .center, spacing: 3) {
            ...
            HStack(alignment: .center, spacing: 2) {
                ForEach(Array(thresholds.enumerated()), id: \.offset) { index, threshold in
                    Capsule()
                        .fill(level >= threshold ? MochiTheme.ink : MochiTheme.divider)
...
private struct TranscriptRow: View {
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 7) {
                ...
                Text(segment.text)
                    ...
                    .foregroundStyle(segment.isFinal ? MochiTheme.text : MochiTheme.secondaryText)
        ...
        .background(
            segment.emphasis == .nameMention
                ? MochiTheme.mint.opacity(namePulse ? 0.95 : 0.58)
                : MochiTheme.surface,
        ...
        .shadow(
            color: segment.emphasis == .nameMention
                ? MochiTheme.ink.opacity(namePulse ? 0.17 : 0.07)
    ...
    private var borderColor: Color {
        if segment.emphasis == .nameMention {
            return MochiTheme.ink.opacity(namePulse ? 0.72 : 0.35)
        }
        if segment.emphasis == .importantDetail { return MochiTheme.amberStrong.opacity(0.55) }
        return MochiTheme.divider.opacity(0.55)
    }
    ...
    private var speakerMarker: some View {
        ...
        .overlay(Circle().stroke(MochiTheme.divider, lineWidth: 1))
    ...
    private var backgroundColor: Color {
        ...
        case .neutral: MochiTheme.surface
        }
    }
    ...
    private func emphasisBadge(_ emphasis: SegmentEmphasis) -> some View {
        ...
        return Label(config.0, systemImage: config.1)
            ...
            .padding(.vertical, 5)
            .background(MochiTheme.surface.opacity(0.8), in: Capsule())
    }
}
...
private struct NameMentionBanner: View {
    ...
    var body: some View {
        HStack(spacing: 14) {
            ZStack {
                Circle()
                    .stroke(MochiTheme.ink.opacity(0.22), lineWidth: 2)
        ...
        .padding(15)
        .background(MochiTheme.mint, in: RoundedRectangle(cornerRadius: 22, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 22).stroke(MochiTheme.ink.opacity(0.28), lineWidth: 1.5))
        .shadow(color: MochiTheme.ink.opacity(0.12), radius: 18, y: 8)
        .onAppear {
...
private struct AttentionBanner: View {
    ...
    var body: some View {
        ...
        .padding(16)
        .background(MochiTheme.amber, in: RoundedRectangle(cornerRadius: 20, style: .continuous))
        .overlay(RoundedRectangle(cornerRadius: 20).stroke(MochiTheme.amberStrong.opacity(0.4), lineWidth: 1))
        .accessibilityElement(children: .contain)
