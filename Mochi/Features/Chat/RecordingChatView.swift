...
struct RecordingChatView: View {
    ...
    private var intro: some View {
        ...
        .padding(14)
        .background(MochiTheme.mint.opacity(0.65), in: RoundedRectangle(cornerRadius: 16))
    }
            ...
            TextField("Ask about this recording", text: $draft, axis: .vertical)
                ...
                .overlay {
                    RoundedRectangle(cornerRadius: 18, style: .continuous)
                        .stroke(MochiTheme.divider, lineWidth: 1)
                }
                .submitLabel(.send)
                .onSubmit { sendDraft() }
    ...
    private func send(_ question: String) {
        ...
        Task {
            do {
                let answer = try await MochiAPI.shared.askRecording(
                    question: question,
...
private struct ChatBubble: View {
    ...
    var body: some View {
        VStack(alignment: message.role == .user ? .trailing : .leading, spacing: 9) {
            Text(message.text)
                .font(.body)
                .foregroundStyle(message.role == .user ? Color.white : MochiTheme.text)
                .padding(.horizontal, 15)
                .padding(.vertical, 12)
                .background(
                    message.role == .user ? MochiTheme.ink : MochiTheme.surface,
