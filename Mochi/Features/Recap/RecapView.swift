        ...
        Button {
            ...
            HStack(alignment: .top, spacing: 13) {
                Image(systemName: recapIcon(item.kind))
                    .font(.subheadline.weight(.bold))
                    .foregroundStyle(item.kind == .unresolved ? MochiTheme.amberStrong : MochiTheme.ink)
                    .frame(width: 38, height: 38)
                    .background(item.kind == .unresolved ? MochiTheme.amber : MochiTheme.mint, in: Circle())
...
private struct FullTranscriptRow: View {
    ...
    private var speakerColor: Color {
        ...
        case .neutral: MochiTheme.secondaryText
        }
    }
}
...
private struct StatusPill: View {
    ...
    private var foreground: Color {
        switch status {
        case .confirmed: MochiTheme.ink
        case .heard: MochiTheme.secondaryText
        case .unresolved: MochiTheme.amberStrong
        }
    }

    private var background: Color {
        switch status {
        case .confirmed: MochiTheme.mint
        case .heard: MochiTheme.surface
        case .unresolved: MochiTheme.amber
        }
    }
}
