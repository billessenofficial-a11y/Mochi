...
struct MochiWordmark: View {
    ...
    var body: some View {
        Text("Mochi")
            ...
            .shadow(color: MochiTheme.mint.opacity(0.9), radius: 0, x: 1.5, y: 2)
        ...
        content
            ...
            .overlay {
                RoundedRectangle(cornerRadius: 22, style: .continuous)
                    .stroke(MochiTheme.divider.opacity(0.65), lineWidth: 1)
            }
    }
}
...
extension View {
    ...
    func mochiScreenBackground() -> some View {
        background {
            LinearGradient(
                colors: [MochiTheme.softMint.opacity(0.82), MochiTheme.canvas],
                startPoint: .top,
                endPoint: .center
            )
            .ignoresSafeArea()
    ...
    var body: some View {
        HStack(spacing: 16) {
            Image("MochiMascot")
                ...
                .padding(5)
                .background(MochiTheme.mint.opacity(0.65), in: Circle())
...
struct PrimaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .background(MochiTheme.ink.opacity(configuration.isPressed ? 0.78 : 1), in: RoundedRectangle(cornerRadius: 18, style: .continuous))
            .scaleEffect(configuration.isPressed ? 0.985 : 1)
            .shadow(color: MochiTheme.ink.opacity(configuration.isPressed ? 0.05 : 0.16), radius: 16, y: 8)
...
struct SecondaryButtonStyle: ButtonStyle {
    func makeBody(configuration: Configuration) -> some View {
        configuration.label
            ...
            .foregroundStyle(MochiTheme.ink)
            .background(MochiTheme.mint.opacity(configuration.isPressed ? 0.6 : 1), in: RoundedRectangle(cornerRadius: 16, style: .continuous))
    }
}
